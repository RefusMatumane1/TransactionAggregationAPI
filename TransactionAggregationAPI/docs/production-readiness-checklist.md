...
| **Helm chart is real, not just scaffolding** | ❌ **Not done** — `k8s/helm/Chart.yaml` and per-environment `values.dev/staging/production.yaml` exist, but there is **no `templates/` directory at all**, so no Helm chart actually renders these values into manifests. The `helm.sh/hook` annotations on `migration-job.yaml` do nothing without a chart to interpret them. The only workflow that actually works today is the plain `kubectl apply` sequence documented in that file's header comment — and it runs against `k8s/configmap.yaml`, which hardcodes `ASPNETCORE_ENVIRONMENT: "development"` for every environment, not just local dev. That matters concretely: the `IsDevelopment()` gate added earlier this review (skip auto-migrate/seed in Staging/Production) only protects a deployment if `ASPNETCORE_ENVIRONMENT` is actually set to something other than Development for that environment — which it currently is not, for any environment using these manifests as-is. | Must-have before treating `k8s/` as ready for a real staging/production deploy |
